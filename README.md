...
会話メッセージをリアルタイムに分析し、内容に合ったスキル（Claude Code のスラッシュコマンド）を自動で検出・提案します。コードレビューを頼めば `code-reviewer` を、調査を頼めば `research` を自動で判別します。
...
会話中のタイムアウト・失敗・応答ムラを自動監視し、負荷が高まると「エージェントを追加しますか？」とダイアログで提案します。承認すると tmux ペインを自動作成してその場で参加させます。
